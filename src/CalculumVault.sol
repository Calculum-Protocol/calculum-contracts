    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        if (
            !_initialAddress[4].isContract() || !_initialAddress[5].isContract()
                || !_initialAddress[6].isContract() || !_initialAddress[7].isContract()
    ...
    function netTransferBalance() private {
        ...
        if ((totalSupply() == 0) && (CURRENT_EPOCH == 0)) {
            ...
            if (
                deposits
                    > withdrawals.add(
                        mgtFee.add(perfFee).mulDiv(
                            TOTAL_VAULT_TOKEN_SUPPLY[CURRENT_EPOCH.sub(1)], 10 ** decimals()
                        )
                    )
            ) {
                actualTx.direction = true;
                ...
                actualTx.amount = withdrawals.add(
                    mgtFee.add(perfFee).mulDiv(
                        TOTAL_VAULT_TOKEN_SUPPLY[CURRENT_EPOCH.sub(1)], 10 ** decimals()
                    )
                ).sub(deposits);
    ...
    function dexTransfer(bool kind) external onlyRole(TRANSFER_BOT_ROLE) nonReentrant {
        ...
        if (actualTx.pending && kind) {
            if (actualTx.direction) {
                ...
                Utils.withdrawVertexCollateral(
                    endpointVertex, address(_asset), 0, uint128(actualTx.amount)
                );
            }
            actualTx.pending = false;
        }
        ...
        if (reserveGas > 0 && !kind) {
            if (_asset.balanceOf(address(this)) < reserveGas) {
                revert Errors.NotEnoughBalance(reserveGas, _asset.balanceOf(address(this)));
            }
    ...
    function feesTransfer() external onlyRole(TRANSFER_BOT_ROLE) nonReentrant {
        ...
        uint256 totalFees = Utils.getPnLPerVaultToken(address(this), address(_asset))
            ...
            : mgtFee.mulDiv(TOTAL_VAULT_TOKEN_SUPPLY[CURRENT_EPOCH.sub(1)], 10 ** decimals());
        uint256 rest =
        ...
        if (rest > 0) {
            SafeERC20Upgradeable.safeTransfer(
                _asset, treasuryWallet, rest - 1 * 10 ** _asset.decimals()
            );
        }
